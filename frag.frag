...
void main() {
    ...
    // Material 11: World-Space Grid Floor (same physical tile size everywhere)
    else if (uMaterialType == 11) {
        float wsTile = 125.0;
        vec2 wsUV = vModelPosition.xz / wsTile;
        vec2 wsGrid = fract(wsUV);
        vec2 wsTk = min(fwidth(wsUV) * 2.0, 0.5);
        vec2 wsEdge = smoothstep(1.0 - wsTk, 1.0 - wsTk * 0.5, wsGrid);
        float wsLine = max(wsEdge.x, wsEdge.y);
        diffuseColor = mix(uBaseColor, vec3(0.05, 0.05, 0.05), wsLine);
        specularStrength = 0.0;
        shininess = 1.0;
    }
